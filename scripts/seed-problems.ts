...
// Categories mapping (slug -> id will be fetched)
...
async function seed() {
  ...
  for (const problem of SEED_PROBLEMS) {
    ...
    if (!categoryId) {
      console.warn(`Category not found: ${problem.category}`);
      continue;
    }
    ...
    if (existing) {
      console.log(`Skipping existing: ${problem.title.substring(0, 40)}...`);
      continue;
    }
    ...
    if (ideaError) {
      console.error(`Error inserting problem: ${problem.title}`, ideaError);
      continue;
    }
    ...
    successCount++;
    console.log(`✅ Added: ${problem.title.substring(0, 50)}...`);
  }

  console.log(`\n🎉 Seed completed! Added ${successCount} problems.`);
}
